  ...
  .define(function (options, adb, router, minitouch, flags) {
    ...
    TouchConsumer.prototype.touchDown = function (point) {
      this._queueWrite(function () {
        ...
        log.info('--------------------touch invoke says: "%s"', chunk)
        // 根据路径信息保存内容，Add by Ivonhoe.Yang
        record(point.recorderPath, chunk);
    ...
    TouchConsumer.prototype.touchCommit = function (point) {
      var chunk = 'c\n'
      // 根据路径信息保存内容，Add by Ivonhoe.Yang
      record(point.recorderPath, chunk);
      this._queueWrite(function () {
    ...
    TouchConsumer.prototype.touchReset = function (point) {
      var chunk = 'r\n'
      // 根据路径信息保存内容，Add by Ivonhoe.Yang
      record(point.recorderPath, chunk);
      this._queueWrite(function () {
      ...
      .then(function (touchConsumer) {
        ...
        router
          ...
          .on(wire.TouchCommitMessage, function (channel, message) {
            queue.push(message.seq, function () {
              touchConsumer.touchCommit(message)
            })
          })
          .on(wire.TouchResetMessage, function (channel, message) {
            queue.push(message.seq, function () {
              touchConsumer.touchReset(message)
            })
          })
