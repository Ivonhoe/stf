module.exports = function ControlServiceFactory($upload
  , $http
  , socket
  , TransactionService
  , $rootScope
  ...
  function ControlService(target, channel) {
    ...
    this.touchDown = function (seq, contact, x, y, pressure) {
      console.log("*******touch down,recorderPath:" + that.recorderPath)
      sendOneWay('input.touchDown', {
        ...
        , y: y
        , pressure: pressure
        , recorderPath: that.recorderPath
      })
    }
    ...
    this.touchUp = function (seq, contact) {
      console.log("*******touch touchMove, contact:" + contact)
    ...
    // 开始屏幕录制，创建日志文件，Add by Ivonhoe.Yang
    this.startRecord = function (path, time, serial, width, height) {
    ...
    this.installFile = function($files) {
      if ($files.length) {
        console.log($files)
    ...
    this.setRecorderPath = function (recorderPath) {
